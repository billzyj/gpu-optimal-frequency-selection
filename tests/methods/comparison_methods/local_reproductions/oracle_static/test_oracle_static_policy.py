...
from src.methods.comparison_methods.local_reproductions.oracle_static import StaticOraclePolicy, SweepPoint, choose_static_oracle_clock
...
class StaticOraclePolicyTests(unittest.TestCase):
    ...
    def test_fallback_summary_includes_profile_target_audit_fields(self) -> None:
        policy = StaticOraclePolicy()
        state = policy.initialize(
            context=make_context(pd_target=0.1),
            config={
        ...
        self.assertAlmostEqual(summary.custom_summary["selected_profile_performance_ratio"], 0.86, places=6)
